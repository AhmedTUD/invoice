...
// API لحذف الفواتير المفلترة فقط (بدون حذف بيانات الموظفين)
app.delete('/api/invoices/filtered', (req, res) => {
  const { sessionToken, filters } = req.body;
  ...
  db.get(
    ...
    (err, session) => {
      ...
      // إذا لم تكن هناك فلاتر، احذف جميع الفواتير
      if (whereConditions.length === 0) {
        db.run('DELETE FROM invoices', (err) => {
          if (err) {
            console.error('خطأ في مسح الفواتير:', err);
            return res.status(500).json({ success: false, message: 'خطأ في مسح الفواتير' });
          }
          ...
          res.json({ success: true, message: 'تم حذف جميع الفواتير بنجاح (بيانات الموظفين محفوظة)' });
        });
        return;
      }
      ...
      const whereClause = whereConditions.join(' AND ');
      const selectQuery = `
        SELECT DISTINCT i.id as invoiceId, i.filePath
        FROM submissions s
        LEFT JOIN invoices i ON s.id = i.submissionId
        WHERE ${whereClause}
      `;
      
      console.log('🔍 استعلام البحث عن الفواتير:', selectQuery);
      console.log('📋 معاملات البحث:', queryParams);
      
      // الحصول على قائمة الفواتير المفلترة
      db.all(selectQuery, queryParams, (err, filteredRows) => {
        if (err) {
          console.error('خطأ في البحث عن الفواتير المفلترة:', err);
          return res.status(500).json({ success: false, message: 'خطأ في البحث عن الفواتير' });
        }
        
        console.log(`📊 عدد الفواتير المفلترة: ${filteredRows.length}`);
        
        if (filteredRows.length === 0) {
          return res.json({ success: true, message: 'لا توجد فواتير تطابق الفلاتر المحددة' });
        }
        
        // جمع معرفات الفواتير والملفات
        const invoiceIds = filteredRows.filter(row => row.invoiceId).map(row => row.invoiceId);
        const filePaths = filteredRows.filter(row => row.filePath).map(row => row.filePath);
        ...
        if (invoiceIds.length > 0) {
          ...
          db.run(
            ...
            function(err) {
              ...
              res.json({ 
                success: true, 
                message: `تم حذف ${invoiceIds.length} فاتورة بنجاح (بيانات الموظفين محفوظة)` 
              });
            }
          );
        } else {
